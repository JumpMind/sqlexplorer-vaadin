...
public class ExportDialog extends ResizableWindow {
    ...
    public ExportDialog(Table table, String filePrefix, String reportTitle) {
        this(filePrefix, reportTitle);
        target = table;
    }

    public ExportDialog(Grid grid, String filePrefix, String reportTitle) {
        this(filePrefix, reportTitle);
        target = grid;
    }
    ...
    protected void csvExport() {
        ...
        if (target instanceof Grid) {
            csvExport = new CsvExport(new DefaultTableHolder((Grid) target));
        } else {
            csvExport = new CsvExport((Table) target);
        }
        csvExport.excludeCollapsedColumns();
        csvExport.setDisplayTotals(false);
        csvExport.setExportFileName(filePrefix + "-export.csv");
    ...
    protected void excelExport() {
        ...
        if (target instanceof Grid) {
            excelExport = new ExcelExport(new DefaultTableHolder((Grid) target));
        } else {
            excelExport = new ExcelExport((Table) target);
        }
        excelExport.excludeCollapsedColumns();
        excelExport.setDisplayTotals(false);
        excelExport.setExportFileName(filePrefix + "-export.xls");
